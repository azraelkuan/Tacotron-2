...
from tacotron.models.attention import compute_attention


class TacotronEncoderWrapper(object):
    """
    Tacotron 2 Encoder Cell
    Pass inputs through a stack of convolutional layers then through a bidirectional LSTM
    layer to predict the hidden representation vector (or memory)
    """

    def __init__(self, conv_layer, lstm_layer):
        super(TacotronEncoderWrapper, self).__init__()
    ...
    def __call__(self, inputs, input_lengths):
        ...
        hidden_representation = self.lstm_layer(conv_output, input_lengths)
...
class TacotronDecoderCellState(
    ...
    the code is adapted from https://github.com/tensorflow/tensorflow/blob/cfd0ea3bfb85d92cdb32760ea024f1e38618d717/
                            tensorflow/contrib/seq2seq/python/ops/attention_wrapper.py#L968-L1021
    !!! Compared the official code, we add a state `finished`
    Contains:
...
class TacotronDecoderWrapper(RNNCell):
    ...
    * : This is typically taking a vanilla LSTM, wrapping it using tensorflow's attention wrapper,
    ...
    def __init__(self, prenet, attention_mechanism, rnn, frame_projection, stop_projection):
        """Initialize decoder parameters
        Args:
            prenet: A tensorflow fully connected layer acting as the decoder pre-net
            attention_mechanism: A _BaseAttentionMechanism instance, useful to
                learn encoder-decoder alignments
            rnn: Instance of RNNCell, main body of the decoder
    ...
    def output_size(self):
        return self._frame_projection.size
    ...
    def state_size(self):
        ...
        return TacotronDecoderCellState(
            cell_state=self.rnn.cell.state_size,
            time=tf.TensorShape([]),
    ...
    def zero_state(self, batch_size, dtype):
        ...
        with tf.name_scope(type(self).__name__ + "ZeroState", values=[batch_size]):
            cell_state = self.rnn.cell.zero_state(batch_size, dtype)
            error_message = (
            ...
            return TacotronDecoderCellState(
                ...
                alignment_history=tf.TensorArray(dtype=dtype, size=0,
                                                 dynamic_size=True)
            )
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        lstm_output, next_cell_state = self.rnn(lstm_input, state.cell_state)
        ...
        context_vector, alignments, cumulated_alignments = compute_attention(self._attention_mechanism,
                                                                             lstm_output,
                                                                             previous_alignments,
