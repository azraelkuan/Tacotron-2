...
class ZoneoutLSTMCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __init__(self, num_units, is_training, zoneout_factor_cell=0., zoneout_factor_output=0., state_is_tuple=True,
        ...
        self._cell = tf.nn.rnn_cell.LSTMCell(num_units=num_units, state_is_tuple=state_is_tuple)
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        # Apply zoneout
        if self.is_training:
            # nn.dropout takes keep_prob (probability to keep activations)
            # not drop_prob (probability to mask activations)!
...
class FrameProjection(object):
    ...
    def __init__(self, size=80, activation=None, scope=None):
        """
        Args:
            size: integer, dimensionality of output space (r*n_mels for decoder or n_mels for postnet)
...
class StopProjection(object):
    ...
    def __init__(self, is_training, size=1, activation=tf.nn.sigmoid, scope=None):
        """
        Args:
            is_training: Boolean, to control the use of sigmoid function as it is useless to use it
                during training since it is integrate inside the sigmoid_cross_entropy loss
            size: integer, dimensionality of output space. Defaults to 1 (scalar)
    ...
    def __call__(self, inputs):
        with tf.variable_scope(self.scope):
            output = tf.layers.dense(inputs, units=self.size,
...
def masked_mse(targets, outputs, targets_lengths, hparams, mask=None):
    ...
    #                               [[1., 1., 1., 0., 0.]],
    #                               [[1., 1., 0., 0., 0.]]]
    # Note the max len argument that ensures mask shape is compatible with r>1
...
def masked_sigmoid_ce(targets, outputs, targets_lengths, hparams, mask=None):
    ...
    with tf.control_dependencies([tf.assert_equal(tf.shape(targets), tf.shape(mask))]):
        # Use a weighted sigmoid cross entropy to measure the <stop_token> loss.
        # Set hparams.cross_entropy_pos_weight to 1
