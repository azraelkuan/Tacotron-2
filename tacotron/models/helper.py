...
class TacoTestHelper(Helper):
    ...
    def next_inputs(self, time, outputs, state, sample_ids, stop_token_prediction, name=None):
        ...
        with tf.name_scope('TacoTestHelper'):
            ...
            # Since we are predicting r frames at each step, two modes are then possible:
            ...
            next_inputs = tf.reshape(next_inputs, [-1, self._output_dim])
...
class TacoTrainingHelper(Helper):
    def __init__(self, batch_size, targets, stop_targets, hparams, gta, evaluating, global_step):
        ...
        with tf.name_scope('TacoTrainingHelper'):
            ...
            self._output_dim = hparams.num_mels
            self._reduction_factor = hparams.outputs_per_step
            self.ratio = tf.convert_to_tensor(hparams.tacotron_teacher_forcing_ratio)
            self.gta = gta
    ...
    def initialize(self, name=None):
        ...
        if self.gta:
            self.ratio = tf.convert_to_tensor(1.)  # Force GTA model to always feed ground-truth
        elif self.eval:
            self.ratio = tf.convert_to_tensor(0.)  # Force eval model to always feed predictions
        else:
            if self._hparams.tacotron_teacher_forcing_mode == 'scheduled':
                self.ratio = _teacher_forcing_ratio_decay(self._hparams.tacotron_teacher_forcing_init_ratio,
    ...
    def next_inputs(self, time, outputs, state, sample_ids, stop_token_prediction, name=None):
        with tf.name_scope(name or 'TacoTrainingHelper'):
            ...
            next_inputs = tf.cond(
                tf.less(tf.random_uniform([], minval=0, maxval=1, dtype=tf.float32), self.ratio),
